...
class FlxAnimate extends FlxSprite
{
	...
	public function new(X:Float = 0, Y:Float = 0, ?Path:String, ?Settings:Settings)
	{
		...
		@:privateAccess
		_pivot = new FlxFrame(FlxGraphic.fromBitmapData(FNFAssets.getBitmapData("flxanimate/images/pivot.png")));
		@:privateAccess
	...
	{
		if (!FNFAssets.exists('$Path/Animation.json') && haxe.io.Path.extension(Path) != "zip")
		{
	...
	function atlasSetting(Path:String):AnimAtlas
	{
		...
		{
			var thing = Zip.readZip(FNFAssets.getBytes(Path));
		...
		{
			jsontxt = haxe.Json.parse(FNFAssets.getText('$Path/Animation.json'));
		}
