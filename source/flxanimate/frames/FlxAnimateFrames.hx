...
class FlxAnimateFrames extends FlxAtlasFrames
{
    ...
    public static function fromTextureAtlas(Path:String):FlxAtlasFrames
    {
        ...
        if (zip != null || haxe.io.Path.extension(Path) == "zip")
        {
            ...
            var thing = (zip != null) ? zip :  Zip.unzip(Zip.readZip(FNFAssets.getBytes(Path)));
			for (list in thing)
			{
        ...
        else
        {
            ...
            {
                var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(FNFAssets.getText('$Path/spritemap.json'), String.fromCharCode(0xFEFF), ""));
                var curSpritemap = FNFAssets.getBitmapData('$Path/${curJson.meta.image}');
            ...
            {
                var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(FNFAssets.getText('$Path/spritemap$i.json'), String.fromCharCode(0xFEFF), ""));
                var curSpritemap = FNFAssets.getBitmapData('$Path/${curJson.meta.image}');
                if (curSpritemap != null)
                {
                    var graphic = FlxG.bitmap.add(curSpritemap);
    ...
    public static function fromSparrow(Path:FlxSparrow, ?Image:FlxGraphicAsset):FlxAtlasFrames
	{
		...
		var data:Access = new Access((Path is String) ? Xml.parse(FNFAssets.getText(Path)).firstElement() : Path.firstElement());
        if (Image == null)
        {
            if ((Path is String))
            {
    ...
    public static function fromJson(Path:FlxJson, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        if ((Path is String) && !Assets.exists(Path))
            return null;
        var data:JsonNormal = (Path is String) ? haxe.Json.parse(FNFAssets.getText(Path)) : Path;
        if (Image == null)
        {
            if ((Path is String))
            {
    ...
    public static function fromStarling(Path:FlxPropertyList, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        if ((Path is String) && !Assets.exists(Path))
            return null;
        var data:Plist = (Path is String) ? PropertyList.parse(FNFAssets.getText(Path)) : Path;
        if (Image == null)
        {
            if ((Path is String))
            {
    ...
    public static function fromCocos2D(Path:String, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        if (!Assets.exists(Path))
            return null;
        var data:Plist = PropertyList.parse(FNFAssets.getText(Path));
    ...
    public static function fromEaselJS(Path:String, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        var separatedJS = FNFAssets.getText(Path).split("\n");
        ...
        for (i in 0...names.length)
        {
            ...
            var name = names[i];
            var json = jsons[i];
            var bitmap = FlxG.bitmap.add(FNFAssets.getBitmapData((Image == null) ? '${imagePath.join("/")}/${json.images[0]}' : Image));
