...
class FlxVideo extends FlxBasic {
	...
	override function update(elapsed:Float) 
	{
		...
		#if web
		if (FlxG.keys.justPressed.E || FlxG.keys.justPressed.P || FlxG.keys.justPressed.Z)
		{
			netStream.dispose();
			if(FlxG.game.contains(player)) FlxG.game.removeChild(player);
	...
	*/
	...
	function fixVolume(e:Event)
	{
		...
		if (FlxG.keys.justPressed.E || FlxG.keys.justPressed.SPACE || FlxG.keys.justPressed.Z)
		{
			vlcBitmap.seek(1);
		}
	}
